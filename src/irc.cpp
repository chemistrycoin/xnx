...
// Copyright (c) 2011-2012 Xanax Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #XanaxTEST0\r");
            Send(hSocket, "WHO #XanaxTEST0\r");
        } else {
            // randomly join #Xanax00-#Xanax99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Xanax%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Xanax%02d\r", channel_number).c_str());
        }
