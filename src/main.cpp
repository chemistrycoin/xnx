...
uint256 hashGenesisBlock("0x800ffe612e567d1e98e690c67e969b651f084a63bd80d823e5038362a0dc89c5");
...
const string strMessageMagic = "Xanax Signed Message:\n";
...
{
    // Xanax: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Xanax: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
	    ...
	    assert(block.hashMerkleRoot == uint256("0xd968cc1694d97a0ca8542f5e81b04a241139ef8e1a2cfc3b7bc4fedaa6a5c592"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0x5a, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XanaxMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XanaxMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static XanaxMiner(CWallet *pwallet)
{
    printf("XanaxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Xanax-miner");
    ...
    try { loop {
        ...
        printf("Running XanaxMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&XanaxMiner, pwallet));
}
